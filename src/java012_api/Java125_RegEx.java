...
public class Java125_RegEx {
	...
	public static void main(String[] args) {
		...
		// a-zA-Z_*$가 sn변수에 포함이 되어 있으면 true or false를 리턴
		...
		System.out.println(sb.matches("2.*")); // ture

		// sb변수에 저장된 값이 2로 시작되고 5로 끝나면 true or false를 리턴
		System.out.println(sb.matches("2.*5")); // ture

		// sb변수에 저장된 값이 a이거나 v로 시작하고 임의문자가 5개이면 true or false를 리턴
		System.out.println(sb.matches("[ab].{5}")); // false

		// st변수에 저장된 값이 a or b로 시작하고 임의문자가 1~5개이며 cc로 끝나면 true or false를 리턴
		st = "aetwefcc";
		System.out.println(st.matches("[ab].{1,5}cc")); // ture
		...
		// input1 = "1253";
		System.out.println(input1.matches("\\d")); // false \d : 숫자 1개를 의미
		Pattern pn = Pattern.compile("\\d");
		Matcher mc = pn.matcher(input1);
		System.out.println(mc);
		System.out.println(mc.find()); // 공백은 0부터 시작을 의미
